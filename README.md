> ⚠️ Questa è la versione **CORSA** (per un secondo profilo). Mettila in un repository **separato**, con i dati Garmin di chi corre nei Secrets. Non mescolarla con quella della bici.

# Apex — il tuo diario Garmin + piano che si aggiorna

Un'app che tieni sulla home del telefono con calendario, storico e piano.

**Due automazioni (per spendere pochissimo):**
- **Ogni giorno** (gratis): scarica i dati Garmin e aggiorna calendario e metriche,
  con una nota "domani" automatica basata sulla prontezza. Nessuna chiamata a Claude.
- **Domenica sera** (~1 chiamata a Claude): analizza la settimana e aggiorna il piano.
  Costo tipico: **meno di 1 €/mese**.

Sono due workflow: `daily.yml` (dati, MODE=daily) e `weekly.yml` (coach, MODE=weekly).
La chiave `ANTHROPIC_API_KEY` serve solo al weekly.
(Facoltativo: notifica WhatsApp/email col riepilogo settimanale.)

**Coppia (facoltativo):** vuoi che il coach veda anche i dati dell'altra persona
(per coordinare riposo e uscite del weekend)? Aggiungi il secret `PARTNER_DATA_URL`
con l'indirizzo dei dati dell'altra app, es. `https://nome.github.io/altro-repo/data`.
